...
static uint32_t tegra_apb_dma_get_mask_mmio(void *opaque)
{
    tegra_apb_dma *s = opaque;

	// "this is the ANDed result of set and clear". what does real hw do?
                                        ...
                                        unsigned size)
{
    ...
    case IRQ_MASK_OFFSET:
        ret = s->irq_mask.reg32 = tegra_apb_dma_get_mask_mmio(s);
        break;
